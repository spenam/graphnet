...
class ReversalLayerF(Function): # As in https://github.com/fungtion/DANN_py3/blob/master/functions.py
...
class DANN_ParticleNeT(GNN):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            domain_classifier: Whether domain classifier branch of the network
                is included. If `True`, the model will include a domain 
                classifier branch to the loss function with the gradient
                reversal layer implemented for back-propagation.
            domain_classifier_layer_sizes: Hidden layer size in the MLP following
                the post-processing _and_ optional global pooling. As this is the
                domain classifier layer, it yields the output of the `DynEdge`.
                Defaults to [256,].
        """
        ...
        # domain_classifier layer sizes
        if domain_classifier_layer_sizes is None:
            domain_classifier_layer_sizes = [
                256,
            ]

        assert isinstance(domain_classifier_layer_sizes, list)
        assert len(domain_classifier_layer_sizes)
        assert all(size > 0 for size in domain_classifier_layer_sizes)
    ...
    def _construct_layers(self) -> None:
        ...
        # Domain-classifier operations
        domain_classifier_layers = []
        domain_classifier_layer_sizes = [nb_latent_features] + list(self._domain_classifier_layer_sizes)
        for nb_in, nb_out in zip(domain_classifier_layer_sizes[:-1], domain_classifier_layer_sizes[1:]):
    ...
    def forward(self, data: Data, alpha: float = 0) -> Tensor  or Tuple[Tensor, Tensor]:
        ...
        if not self._skip_readout:
            ...
            # Read-out
            x_readout = self._readout(x)

            # domain_classifier
            if self._domain_classifier:
                x_domain_classifier = ReversalLayerF.apply(x, alpha)
                x_domain_classifier = self._domain_classifier(x)
                return x_readout, x_domain_classifier
