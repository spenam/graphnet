...
class KM3NeTMCTruthExtractor(KM3NeTROOTExtractor):
    ...
    def _extract_truth_dataframe(self, file: Any) -> Any:
        ...
        if (len(file.mc_trks.pdgid[0]) == 2) and (file.mc_trks.pdgid[0][0] == file.mc_trks.pdgid[0][1]) and (file.mc_trks.pdgid[0][0] in [11, 111, 211]):
            ...
            #compute the energy imbalance

            Energy_imbalance = (file.mc_trks.E[:, 0] - file.mc_trks.E[:, 1]) / (file.mc_trks.E[:, 0] + file.mc_trks.E[:, 1])

            zen_second, az_second = xyz_dir_to_zen_az(
            np.array(file.mc_trks.dir_x[:, 1]),
            np.array(file.mc_trks.dir_y[:, 1]),
            np.array(file.mc_trks.dir_z[:, 1]),
            padding_value,
        )
            ...
            dict_truth = {
                ...
                "vrx_x_second": np.array(file.mc_trks.pos_x[:, 1]),
                "vrx_y_second": np.array(file.mc_trks.pos_y[:, 1]),
                "vrx_z_second": np.array(file.mc_trks.pos_z[:, 1]),
                "zenith": zen_truth,
                "azimuth": az_truth,
                "zenith_second": zen_second,
                "azimuth_second": az_second,
                "part_dir_x": part_dir_x,
                "part_dir_y": part_dir_y,
                "part_dir_z": part_dir_z,
                "part_dir_x_second": np.array(file.mc_trks.dir_x[:, 1]),
                "part_dir_y_second": np.array(file.mc_trks.dir_y[:, 1]),
                "part_dir_z_second": np.array(file.mc_trks.dir_z[:, 1]),
                "Energy": np.array(primaries.E),
                "Energy_second": np.array(file.mc_trks.E[:, 1]),
            ...
            dict_truth = {
                ...
                "vrx_y": np.array(primaries.pos_y),
                "vrx_z": np.array(primaries.pos_z),
                "vrx_x_second": padding_value*np.ones(len(primaries.pos_x)),
                "vrx_y_second": padding_value*np.ones(len(primaries.pos_x)),
                "vrx_z_second": padding_value*np.ones(len(primaries.pos_x)),
                "zenith": zen_truth,
                "azimuth": az_truth,
                "zenith_second": padding_value*np.ones(len(primaries.pos_x)),
                "azimuth_second": padding_value*np.ones(len(primaries.pos_x)),
                "part_dir_x": part_dir_x,
                "part_dir_y": part_dir_y,
                "part_dir_z": part_dir_z,
                "part_dir_x_second": padding_value*np.ones(len(primaries.pos_x)),
                "part_dir_y_second": padding_value*np.ones(len(primaries.pos_x)),
                "part_dir_z_second": padding_value*np.ones(len(primaries.pos_x)),
                "Energy": np.array(primaries.E),
                "Energy_second": padding_value*np.ones(len(primaries.pos_x)),
                "is_double_cascade": is_double_cascade,
                "distance": np.array(distance),
                "Energy_imbalance": np.ones(len(primaries.pos_x), dtype=float),
                "event_no": np.array(unique_id).astype(int),
            }
